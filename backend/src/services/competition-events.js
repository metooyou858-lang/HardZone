...
async function updateCompetitionEvent(eventKey, input) {
  return withTransaction(async client => {
    ...
    if (scheduledKeys.filter(Boolean).some(key => !settings.categories.some(category => category.key === key))) throw invalid('Нельзя удалить категорию с комплексами или сохранённой сеткой');
