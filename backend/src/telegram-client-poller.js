...
async function poll() {
  if (!isEnabled()) {
    logger.info('telegram_client', { action: 'poller_disabled' });
    return;
  }

  if (!process.env.TELEGRAM_CLIENT_BOT_TOKEN) {
    logger.warn('telegram_client', { action: 'poller_disabled_missing_token' });
    return;
  }
  ...
  while (!stopped) {
    try {
      const result = await telegramClientRequest(
        'getUpdates',
        {
          offset,
...
async function shutdown(signal) {
  stopped = true;
  logger.info('telegram_client', { action: 'poller_stopping', signal });
  await pool.end();
  process.exit(0);
}
