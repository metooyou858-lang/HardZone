...
function PayrollRulesPanel({ rules, trainers, trainingTypes, services, busy, onCreate, onUpdate, onDelete }: {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setEditingId(null);
    setName(""); setTrainerIds([]); setTrainingTypeIds([]); setProductIds([]); setBaseAmount(""); setBonusThreshold(""); setBonusPerPerson(""); setPerAttendeeAmount(""); setPercentageRate(""); setTiers([{ from: "1", to: "", amount: "" }]); setComment("");
  }

  function startEdit(rule: PayrollRule) { setEditingId(rule.id); setName(rule.name); setAllTrainers(rule.all_trainers); setTrainerIds(rule.trainers.map((t)=>Number(t.trainer_id))); setAllActivities(rule.all_activities); setTrainingTypeIds(rule.items.filter((i)=>i.training_type_id!==null).map((i)=>Number(i.training_type_id))); setProductIds(rule.items.filter((i)=>i.product_id!==null).map((i)=>Number(i.product_id))); setCalculationType(rule.calculation_type); setBaseAmount(String(rule.base_amount || "")); setBonusThreshold(rule.bonus_threshold===null?"":String(rule.bonus_threshold)); setBonusPerPerson(String(rule.bonus_per_person || "")); setPerAttendeeAmount(String(rule.per_attendee_amount || "")); setPercentageRate(String(rule.percentage_rate || "")); setTiers(rule.tiers.length ? rule.tiers.map((t)=>({from:String(t.from),to:t.to===null?"":String(t.to),amount:String(t.amount)})) : [{from:"1",to:"",amount:""}]); setEffectiveFrom(rule.effective_from); setComment(rule.comment || ""); window.scrollTo({top:0,behavior:"smooth"}); }

  function ruleFormula(rule: PayrollRule) {
    const base = `Оклад ${formatMoney(rule.base_amount)} за занятие`;
    if (rule.calculation_type === "per_attendee") return `${base} · +${formatMoney(rule.per_attendee_amount)} за каждого пришедшего`;
    if (rule.calculation_type === "percentage") return `${base} · +${rule.percentage_rate}% тренеру · ${100 - rule.percentage_rate}% залу`;
    if (rule.calculation_type === "tiered") return `${base} · ` + rule.tiers.map((tier) => `${tier.from}–${tier.to ?? "∞"}: +${formatMoney(tier.amount)}`).join(" · ");
    return `${base}${rule.bonus_threshold === null ? "" : ` · свыше ${rule.bonus_threshold}: +${formatMoney(rule.bonus_per_person || 0)}`}`;
  }
