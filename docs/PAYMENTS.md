...
### Срок оплаты и уведомления (production, 11.09.2026)
...
- через 60 минут выполняется `GetState` всех попыток; заявку можно освобождать только после подтверждения конечного неоплаченного состояния каждой попытки;
- при недоступности Т-Банка автоматическая отмена откладывается;
- после `CONFIRMED` будущие письма об неоплате отменяются, создаётся одно задание подтверждения; неопределённый SMTP-ответ автоматически не повторяется;
- `Init` получает email для чека и `RedirectDueDate`; незавершённый статус банка удерживает заявку даже после срока. `Cancel` не вызывается, чтобы гонка с подтверждением оплаты не создала возврат;
- исторические заявки без `payment_deadline` исключены из автоматизации.

Подробный контракт и проверки: `docs/COMPETITION_AUTOMATION.md`.
