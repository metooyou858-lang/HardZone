...
### Ручное восстановление V4-фискализации

- Кнопка «Восстановить чек» в истории продаж обращается к `/sync-aqsi`. V4-router перехватывает этот URL для заказов со слипом/операцией эквайринга и вызывает `syncAqsiV4`; legacy/наличные без этих признаков остаются на прежнем маршруте. Проверять необходимо HTTP-путь кнопки, а не только сервис `/sync-aqsi-v4`.

- `sync-aqsi-v4` повторяет `Canceled`, `Error` и `Timeout` при сохранённом слипе с одобренной покупкой на сумму заказа и отсутствии фискальных данных. Причина ошибки не является запретом на ручной повтор: актуальный результат возвращает касса.
- Для всех семи статусов AQSI (`Pending`, `Processing`, `Finishing`, `Completed`, `Canceled`, `Timeout`, `Error`) полные фискальные реквизиты или найденный по слипу чек восстанавливают продажу без повторной отправки. При активной операции без чека возвращается ожидание; `Completed` без реквизитов, неизвестный статус, неполные фискальные данные и недоступность API требуют сверки, а не создания дубликата.
